...
import {RETELL_API_KEY_WSC} from '../config/constants';

export class SellMyCarController {
    static async createWebCall(req: Request, res: Response): Promise<void> {
        ...
        const response = await httpClientPlugin.post('https://api.retellai.com/v2/create-web-call', payload, {
            headers: {
                Authorization: `Bearer ${RETELL_API_KEY_WSC}`, // Replace with your actual Bearer token
